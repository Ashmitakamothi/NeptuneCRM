...
import { Home, ChevronRight, Download, Search } from 'lucide-react';
    ...
    <div className="animate-fade-in space-y-4 pb-10">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 py-4 border-b border-[var(--border-color)]">
        <div className="flex items-center gap-3">
          <h1 className="text-[22px] font-bold uppercase tracking-tight text-white">{t.deposit}</h1>
          <div className="flex items-center gap-2">
             <div className="w-2.5 h-2.5 bg-[#AF6C56] rounded-full dotanim"></div>
             <div className="bg-[#12716E] text-white text-[13px] font-medium px-3 py-1 rounded-[4px]">
                {t.news}
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="hidden md:flex items-center gap-3 bg-[#111818] border border-white/5 px-5 py-2 rounded-xl">
             <p className="text-[14px] font-medium text-white/70">My Referral Link: <span className="text-white">IFAHGGAP</span></p>
          </div>
          
          <div className="flex bg-[var(--sub-bg)] rounded-lg p-1 border border-[var(--border-color)]">
            <button 
              onClick={() => onNavigate('Dashboard')}
              className="px-4 py-1.5 rounded-md text-sm font-medium text-[#8e9d9b] hover:text-white transition-all"
            >
              User Dashboard
            </button>
            <button className="px-4 py-1.5 rounded-md text-sm font-medium bg-[#12716E] text-white shadow-lg">
              IB Dashboard
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-2">
         <Breadcrumb
           ...
           items={[
             ...
             { title: <span className="text-[var(--text-color)] font-medium">{t.deposit}</span> },
           ]}
         />
