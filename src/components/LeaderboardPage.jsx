...
const LeaderboardPage = ({ onNavigate }) => {
  ...
  return (
    <div className="flex flex-col w-full h-[calc(100vh-100px)] animate-fade-in pb-20">
      {/* Top Header Bar */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-white/5 mb-6 shrink-0">
        <div className="flex items-center gap-3">
          <h1 className="text-[20px] md:text-[24px] font-extrabold text-white tracking-tight leading-none uppercase">LEADERBOARD</h1>
      ...
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-[15px] mb-6 font-medium shrink-0">
        ...
        <span className="text-white cursor-default tracking-wide">Leaderboard</span>
      </div>

      {/* Empty State Centered */}
      <div className="flex-1 flex flex-col items-center justify-center -mt-20">
        <img src={noDataImage} alt="No Data" className="w-[280px] md:w-[350px] mb-6 opacity-90" />
        <h2 className="text-[26px] md:text-[30px] font-bold text-[#3B4E68] mb-3 text-center">No Leaderboard Data Yet</h2>
        <p className="text-[#8e9d9b] text-[15px] md:text-[16px] font-medium text-center">There is no data to show you right now</p>
      </div>
    </div>
  );
};
